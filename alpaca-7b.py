...
checkpoint = "/home/ubuntu/benchmarking/alpaca-lora-7b"
config = LlamaConfig()
...
model = load_checkpoint_and_dispatch(model, "alpaca-lora-7b", device_map=device_map)
...
for i, conv in enumerate(data):
    ...
    response = outputs[0].split(prompt)[-1]
    response = response.split("\nYou:")[0].strip()
    benchmark_output[i] = [{"prompt": prompt, "response": response, "model": "chainyo/alpaca-lora-7b"}]
    times += (end - start)
...
pkl.dump(benchmark_output, open("benchmark_output_alpaca_7b_lora.pkl", "wb"))
