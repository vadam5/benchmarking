...
for i, conv in enumerate(data):
    ...
    response = outputs[0].split(prompt)[-1]
    response = response.split("\nYou:")[0].strip()
    benchmark_output[i] = [{"prompt": prompt, "response": response, "model": "PygmalionAI/pygmalion-350m"}]

    print(prompt)
    print(response)

pkl.dump(benchmark_output, open("benchmark_output_pygmalion-350m.pkl", "wb"))
