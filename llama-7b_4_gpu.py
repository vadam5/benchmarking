...
checkpoint = "/home/ubuntu/benchmarking/llama-7b-hf"
config = LlamaConfig()
...
model = load_checkpoint_and_dispatch(model, "llama-7b-hf", device_map=device_map)
...
for i, conv in enumerate(data):
    ...
    response = outputs[0].split(prompt)[-1]
    response = response.split("\nYou:")[0].strip()
    benchmark_output[i] = [{"prompt": prompt, "response": response, "model": "decapoda-research/llama-7b-hf"}]
    times += (end - start)
...
pkl.dump(benchmark_output, open("benchmark_output_llama_7b_hf.pkl", "wb"))
